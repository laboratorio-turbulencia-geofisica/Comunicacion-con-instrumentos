...
#initialize Daqx.dll
dll = r"C:\Program Files (x86)\DaqX\Drivers\USB_x64\DaqX64.dll" 
daq = ct.OleDLL(dll)
...
def FormatError(errNum):
    ...
    msg = (ct.c_char * 64)()  # Messages are at least 64 characters
    daq.daqFormatError(errNum, ct.byref(msg))
    
    return msg.value.decode(errors="replace")  # Decode bytes to string safely
...
class daqDevice(object):
    ...
    def AdcSetDiskFile(self, filename, openMode, preWrite):
        ...
        filename_c = ct.c_char_p(filename.encode('utf-8'))

        err = daq.daqAdcSetDiskFile(self.handle, filename_c, openMode, preWrite)
    ...
    def AdcRdScan(self, startChan, endChan, gain, flags, convert = None):
        ...
        err = daq.daqAdcRdScan(self.handle, wt.DWORD(startChan), wt.DWORD(endChan),
                         pbuf, wt.DWORD(gain), wt.DWORD(flags))
